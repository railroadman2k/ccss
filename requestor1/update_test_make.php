...
elseif(array_key_exists("selected_ticket_id", $_SESSION))
{
    ...
    $group_of_tickets = 0;
    $xtpl->assign("TICKET_ID", "<input type=\"hidden\" name=\"select[]\" value=\"" . $ticket_id . "\">");
}
else $ticket_id = 0;
    ...
    if(!$group_of_tickets)
    {
        ...
        if($body_id)
        {
            ...
            if(__CFG_SEND_EMAIL && $_REQUEST["message_status"] == "reply")
            {
                ...
                $domen = substr($_SESSION["website_url"],7,-1);
                ...
                $headers .= "From: {$row1['name']}@$domen <{$row1['name']}@$domen>\r\n";
                $message = $_REQUEST["message"] . <<<sss
...
Attention:
    ...
    {$_SESSION["website_url"]}support.rpx?id=$ticket_id
...
sss;
         ...
         //       mail($row["from_email"], $subject, $message, $headers);
         		  $act="update";
            }
...
$query = "SELECT id, name FROM " . T_TEMPLATES . " WHERE template_website='" . $_SESSION["website_url"] . "'";
$rows = SQL_select($query);
