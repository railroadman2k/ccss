...
function createHASH($mid)
{
	$rand1 = rand (2000000,4000000);
	$rand2 = rand (4000000,9000000);
	$rand3 = $mid;
	$rand_all = $rand1 . $rand2 . $rand3;
	$rand_all = md5($rand_all);
	return $rand_all;
}

function setHASH($ticket_id,$uid,$hash)
{
	$sql = "insert into confident.letters set hash='{$hash}', uid={$uid},mailid={$ticket_id}, date=NOW()";
	mysql_query($sql) or die("cant add record to confident.mailing on ".__CLASS__."#".__METHOD__."#".__LINE__);
	return true;
}

function setSentDate($timestamp = null)
{
	if ($timestamp == null)
	{
		$data['datetime_format']=date('Y-m-d H:i:s');
		$data['datetime_line']=date('YmdHis');
	}
	else
	{
		$data['datetime_format']=date('Y-m-d H:i:s', $timestamp);
		$data['datetime_line']=date('YmdHis', $timestamp);
	}
	
	return $data;
}

function addEmailHash($uid, $type, $hash=null, $date)
{
	$date = $date["datetime_format"];
	$rand1 = rand (2000000,4000000);
	$rand2 = rand (4000000,9000000);
	$rand_all = $rand1 . $rand2 . $uid;
	$hash_mail = md5($rand_all);

	$sql = "INSERT INTO confident.mailing SET uid='{$uid}',type= '{$type}',date_create='{$date}',hash_mail='{$hash_mail}'";
	mysql_query($sql) or die("cant add record to confident.mailing on ".__CLASS__."#".__METHOD__."#".__LINE__);
	if($hash!=null)
	{
		$sql = "INSERT INTO confident.mailing_letters (hash_mail, hash_letters) VALUES ('".$hash_mail."', '".$hash."')";
		mysql_query($sql) or die("cant add record to confident.mailing_letters on ". __CLASS__."#".__METHOD__."#".__LINE__);
	}
	return $hash_mail;
}
...
if($act == "add_message" && strlen(trim($_REQUEST["message"])) > 2)
{
		//mailstatistic
		$date = setSentDate();
		$hash = createHASH($ticket_id);
		setHASH($ticket_id,$row["user_id"],$hash);
		addEmailHash($row["user_id"], 'requestor', $hash,$date);
		echo "here";exit();
...
You can send your answer to your question on our site.
    ...
    Thank you for using our services,
    	CC Support Team,
	{$_REQUEST["message"]}
<img src="http://www.ualadys.com/letter.jpg/%hash_mail%" alt=""  border="0"  width="1" height="1">
</html>
...
<html>
<head>
  <title>ANSWER: {$subject}/[CCSS:{$ticket_id}]</title>
</head>
<body bgcolor="#FFFFFF">
...
if(!$group_of_tickets)
{
    ...
    $xtpl->parse("main.update12.ticket_info");
    ...
    if($row2 && trim($row2["body"]) != "")
    {
        ...
        $xtpl->parse("main.update12.reply_message");
    }
