...
function setHASH($mid,$to_id,$hash)
{
	$sql = "insert into letters set hash='{$hash}', uid={$to_id},mailid={$mid}, date=NOW()";
	mysql_query($sql);
	return true;
}
...
function setSentDate($timestamp = null)
{
	if ($timestamp == null)
	{
		$this->datetime_format=date('Y-m-d H:i:s');
		$this->datetime_line=date('YmdHis');
	}
	else
	{
		$this->datetime_format=date('Y-m-d H:i:s', $timestamp);
		$this->datetime_line=date('YmdHis', $timestamp);
	}
}
